...
char *publishAPI(char* reqbody,char* url,int memorySize)
{
         ...
         struct response_data *res_data=publishAPIRequest(reqbody,cookie,memorySize,url);
         if(res_data==NULL){
         ...
         char* dbstatus=databaseHandlerAPIPublish(reqbody,res_data->res_cookie);
         if(dbstatus==NULL)
         {
              return NULL;
         }
         printf("{error:\"false\",message:\"Publishing an API is done successfully\"}\n");
         return "Ok";
}
